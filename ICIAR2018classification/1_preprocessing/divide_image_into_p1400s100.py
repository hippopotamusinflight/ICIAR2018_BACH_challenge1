...
image_label_df["class_num"] = image_label_df.apply(lambda row: tumor_label(row), axis=1)


# make dir
photos_dir = "/global/home/hpc4535/CISC881FinalProject/ICIAR2018_BACH_Challenge/Photos/"
all_patches1400_dir = os.path.join(photos_dir, "all_patches1400/")

if not os.path.exists(all_patches1400_dir):
    os.makedirs(all_patches1400_dir)
else:
...
patches_and_label_1400_stride100_df = pd.DataFrame()
...
#     bkgd_patches = []
    ...
    for i in range(img_patches_B.shape[0]):
        for j in range(img_patches_B.shape[1]):
            img_patch = np.dstack((img_patches_R[i,j], img_patches_G[i,j], img_patches_B[i,j]))
...
#                 bkgd_patches.append(img_patch)
    ...
    for i, patch in enumerate(all_patches):
        ...
        f_name = all_patches1400_dir + os.path.splitext(row['tif'])[0] + "_" + str(i).zfill(3) + ".tif"
        im.save(f_name)
        patches_and_label_1400_stride100_df = patches_and_label_1400_stride100_df.append({'filename': (os.path.splitext(row['tif'])[0] + "_" + str(i).zfill(3) + ".tif"), "class_num": row['class_num']}, ignore_index=True)
    
    print(row['tif'], "patch extraction done.")

patches_and_label_1400_stride100_df['class_num'] = patches_and_label_1400_stride100_df['class_num'].astype(int)
print(patches_and_label_1400_stride100_df.shape)

all_patches1400_dir = os.path.join(photos_dir, "all_patches1400/")
df_name = "patches_and_label_1400_stride100.csv"

patches_and_label_1400_stride100_df.to_csv(os.path.join(all_patches1400_dir, df_name), sep='\t')
