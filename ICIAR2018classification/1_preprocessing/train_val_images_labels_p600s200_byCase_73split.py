...
all_patches_dir = os.path.join(photos_dir, "all_p600s200/")
patches_labels_path = os.path.join(all_patches_dir, "patches_and_labels_p600s200.csv")

resize = 300
'''

def get_train_val_arrays(images_labels_path, patches_labels_path, all_patches_dir, resize):
...
#                 test_image_arr_list.append(img_RGB_resz)
    ...
    np.save("./X_train_p600s200_byCase_73split", X_train)
    np.save("./y_train_p600s200_byCase_73split", y_train)
    
    X_val = np.array(val_image_arr_list)
    y_val = np.array(val_label_list)
    y_val = np_utils.to_categorical(y_val)
    np.save("./X_val_p600s200_byCase_73split", X_val)
    np.save("./y_val_p600s200_byCase_73split", y_val)      
    
#     X_test = np.array(test_image_arr_list)
#     y_test = np.array(test_label_list)
#     y_test = np_utils.to_categorical(y_test)
#     np.save("./X_test_1400_byCase", X_test)
#     np.save("./y_test_1400_byCase", y_test)   
    ...
    validation_images_labels_df.to_csv("./validation_images_labels_df_73split_p600s500.csv", index=False)
...
if __name__ == "__main__":
    ...
    get_train_val_arrays(images_labels_path, patches_labels_path, all_patches_dir, resize)
