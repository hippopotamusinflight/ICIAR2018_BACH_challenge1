...
all_p600s200_dir = os.path.join(photos_dir, "all_p600s200/")

if not os.path.exists(all_p600s200_dir):
    os.makedirs(all_p600s200_dir)
else:
...
# index, row = next(image_label_df.iterrows())
    ...
    for i, patch in enumerate(non_bkgd_patches):
        ...
        f_name = all_p600s200_dir + os.path.splitext(row['tif'])[0] + "_" + str(i).zfill(3) + ".tif"
        im.save(f_name)
...
# write csv
all_p600s200_dir = os.path.join(photos_dir, "all_p600s200/")
df_name = "patches_and_labels_p600s200.csv"

patches_label_df.to_csv(os.path.join(all_p600s200_dir, df_name), sep='\t')
